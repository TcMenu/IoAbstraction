...
class SwitchesFixture : public UnitTestExecutor {
protected:
...
public:
    ...
    void assertPressedState(bool shouldBePressed) {
        ...
        assertEquals(pressed, shouldBePressed);
    ...
    void assertHeldState(bool shouldBeHeld) {
        ...
        assertEquals(held, shouldBeHeld);
        assertFalse(keyReleased);
        assertEquals(mockIo.getErrorMode(), NO_ERROR);
    }
    ...
    void assertReleasedState(bool expectedHeldState) {
        ...
        assertEquals(held, expectedHeldState);
    }
...
testF(SwitchesFixture, testPressingASingleButton) {
    ...
    assertMoreThan((uint32_t)380, safeMilliDiffFromNow(millisStart));
    assertLessThan((uint32_t)450, safeMilliDiffFromNow(millisStart));
}
...
testF(SwitchesFixture, testInterruptButtonRepeating) {
    ...
    assertEquals(callsMade, callsWhenButtonReleased);
}
...
testF(SwitchesFixture, testUpDownEncoder) {
    ...
    assertEquals(encoderCurrentVal, 10);
    ...
    // and zero reading.
    assertEquals(encoderCurrentVal, 0);
...
testF(SwitchesFixture, testChangingCallbacks) {
    ...
    assertEquals(callsMade, oldNumberOfCalls);
    switches.setEncoder(0, nullptr);
}
